-- FloatChat ARGO Database Schema for Supabase
...
-- Create Vector Embeddings table for RAG system
CREATE TABLE IF NOT EXISTS vector_embeddings (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    content_id VARCHAR(100) NOT NULL,
    content_type VARCHAR(50) NOT NULL,
    content_text TEXT NOT NULL,
    embedding TEXT, -- Store as JSON array string if VECTOR type not available
...
-- Public read access to ARGO data
CREATE POLICY "Public read access to ARGO floats" ON argo_floats
    FOR SELECT USING (true);

CREATE POLICY "Public read access to ARGO profiles" ON argo_profiles
    FOR SELECT USING (true);

CREATE POLICY "Public read access to data sources" ON data_sources
    FOR SELECT USING (true);

-- Service role full access (for backend operations)
CREATE POLICY "Service role full access to all tables" ON argo_floats
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to profiles" ON argo_profiles
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Service role full access to data_processing_jobs" ON data_processing_jobs
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to data_sources" ON data_sources
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to vector_embeddings" ON vector_embeddings
    FOR ALL USING (auth.role() = 'service_role');

-- Grant permissions
GRANT USAGE ON SCHEMA public TO anon, authenticated, service_role;
GRANT ALL ON ALL TABLES IN SCHEMA public TO service_role;
GRANT SELECT ON argo_floats TO anon, authenticated;
GRANT SELECT ON argo_profiles TO anon, authenticated;
